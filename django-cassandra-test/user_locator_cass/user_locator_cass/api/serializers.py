...
from .models import Account, AdminUser, UsLocation
import warnings
...
#class CassandraSerializer(serializers.ModelSerializer):
...
#class AccountSerializer(CassandraSerializer):
class AccountSerializer(DjangoCassandraModelSerializer):
    ...
    class Meta:
        model = Account
        fields = ('username', 'first_name', 'last_name', 'email', 'password', 'phone', 'gender','created_date')

#class LocationSerializer(CassandraSerializer):
class LocationSerializer(DjangoCassandraModelSerializer):

    class Meta:
        model = UsLocation
        fields = ('city_name', 'zip_code')

#class AdminUserSerializer(CassandraSerializer):
class AdminUserSerializer(DjangoCassandraModelSerializer):
    ...
    #     user_obj = AdminUser.objects.filter(username=attrs.get(self.username_field)).first() or AdminUser.objects.filter(
    ...
    #             #user = get_object_or_404(queryset, **credentials)
    #             user = AdminUserSerializer(queryset)
    #             if user:
    #                 #if not user.is_active:
    #                 if user is None :
    #                     # CT msg = _('User account is disabled.')
